...
from src.QuotationSubProductEmployee.models import QuotationSubProductEmployeeCreate , QuotationSubProductEmployeeRead ,QuotationSubProductEmployeeDelete ,QuotationSubProductAddRemark,QuotationSubProductQuantity
from src.QuotationSubProductEmployee.service import create , get_subproducts_by_product_id ,delete_subproduct_by_id , add_remark_byid,add_quantity_byid
...
def add_remark(
    quantity: QuotationSubProductQuantity, 
    auth_token: str = Header(None, convert_underscores=True, alias="AuthToken"),
    db: Session = Depends(get_db)
):
    if auth_token != get_token():
    ...
    return add_quantity_byid(db=db, quantity=quantity)
